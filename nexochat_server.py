...
class NexoHandler(BaseHTTPRequestHandler):
    ...
    def do_OPTIONS(self):
        ...
        if origin in ALLOWED_ORIGINS:   # FIX #5
    ...
    def _handle_ws_upgrade(self):
        ...
        with WS_LOCK:
            WS_CLIENTS.setdefault(chat_id, set()).add(ws_conn)
...
class SimpleWSConnection:
    ...
    def _build_frame(self, data: bytes) -> bytes:
        ...
        elif ln < 65536:  hdr += bytearray([126]) + struct.pack(">H", ln)
        else:             hdr += bytearray([127]) + struct.pack(">Q", ln)
...
def build_auth_page(lang="en") -> str:
    """Auth page served by the Python server at /auth.
    Uses correct /api/auth/* endpoints (no /web/ prefix)."""
...
def build_main_page(user: dict, lang="en", active_chat=None) -> str:
    """Minimal redirect — the main page logic is in the server-rendered HTML below.
    For a full SPA this would embed the entire chat interface."""
