...
SITE_ADMIN      = "puwe"
SITE_ADMIN_PASS = "93059306"

# ── Email (SMTP) config ───────────────────────────────────────────
SMTP_EMAIL    = os.environ.get("SMTP_EMAIL", "")      # your@gmail.com
SMTP_PASSWORD = os.environ.get("SMTP_PASSWORD", "")   # Gmail App Password
SMTP_HOST     = os.environ.get("SMTP_HOST", "smtp.gmail.com")
SMTP_PORT     = int(os.environ.get("SMTP_PORT", "465"))
SITE_URL      = os.environ.get("NEXO_URL", "https://nexochat.bothost.tech")

RESET_TOKENS_F = DATA / "reset_tokens.json"  # {token: {username, expires}}
...
def seed():
    ...
    if SITE_ADMIN not in users:
        users[SITE_ADMIN] = {
            ...
            "username": SITE_ADMIN,
            "email": "",
            "password_hash": hash_pw(SITE_ADMIN_PASS),
            "display": "Puwe",
            "bio": "Admin",
            "avatar": "",
            "avatar_emoji": "👑",
...
def send_recovery_email(to_email: str, reset_token: str) -> bool:
    ...
    msg["To"]      = to_email
    html = f"""
    <div style="font-family:sans-serif;max-width:480px;margin:auto;background:#141210;color:#f5ede4;padding:32px;border-radius:16px;">
      <h2 style="color:#ff8c32;margin-bottom:8px;">🔐 Password Reset</h2>
      <p>You requested a password reset for your NexoChat account.</p>
      <a href="{reset_link}"
         style="display:inline-block;margin:20px 0;padding:12px 28px;background:linear-gradient(135deg,#ff6a00,#ff8c32);
    ...
    try:
        ...
        log.info(f"Recovery email sent to {to_email}")
        return True
...
def build_main_page(user: dict, lang="en", active_chat=None) -> str:
    ...
    # Pre-compute emoji pickers (backslashes not allowed inside f-strings in Python < 3.12)
    def _emoji_span(e, fn):
        return f'<span class="emoji-opt" onclick="{fn}(this,&quot;{e}&quot;)">{e}</span>'
    def _react_span(e):
        return f'<span class="r-emoji" onclick="addReaction(&quot;{e}&quot;)">{e}</span>'

    chat_emojis = "".join(_emoji_span(e, "selectEmoji")
        for e in ['💬','👥','🔥','🌍','🎮','🎵','📸','🚀','⚡','🌙','💡','🎯','🏆','🦄','🌈'])
    profile_emojis = "".join(_emoji_span(e, "selectProfileEmoji")
        for e in ['👤','😎','🦁','🐉','🌟','🔥','❄️','🎭','🎨','🚀','👑','🦅','🌙','⚡','🎯'])
    reaction_emojis = "".join(_react_span(e)
        for e in ['👍','❤️','😂','😮','😢','🔥','👏','🎉','😡','💯'])
...
def main():
    ...
    # ── Бот у фоновому потоці (тільки якщо не запущений як subprocess сервера) ──
    if not os.environ.get("NEXO_BOT_SUBPROCESS"):
        def _run_bot():
            bot_file = BASE_DIR / "nexochat_bot.py"
            if not bot_file.exists():
                log.warning("nexochat_bot.py не знайдено — бот не запущено")
                return
            env = os.environ.copy()
            env["NEXO_BOT_SUBPROCESS"] = "1"  # щоб бот не запускав сервер знову
            ...
            while True:
                try:
                    ...
                    proc.wait()
                    log.warning("Бот зупинився (код %d), рестарт через 5с...", proc.returncode)
                    time.sleep(5)
                except Exception as e:
                    log.warning("Помилка запуску бота: %s", e)
                    time.sleep(5)
